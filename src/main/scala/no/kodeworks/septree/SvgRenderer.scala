...
object SvgRenderer extends App {
  ...
  def renderPaths(sepTree: SepTree, transformY: Double => Double = identity) = {
    ...
    //    )
    //    val lines = Array(
    //      Line(Point(sepTree.space.lowerLeft.x, sepTree.space.upperRight.y), Point(sepTree.space.upperRight.x, sepTree.space.lowerLeft.y)),
    //      Line(sepTree.space.lowerLeft, sepTree.space.upperRight)
    //    )
    //    val lines = SvgParser.svgStringToLinesScaledToSpace(Source.fromFile("C:\\dev\\assets\\KW_symbol_green_RGB.svg").mkString, Some(Space(Point(0d, 0d), Point(21003d, 29702d))), 0.5)
    //
    //    var s = System.currentTimeMillis()
    //    val lineIndices = lines.map(hex.indexLine)
    //    println(s"lineIndices took ${System.currentTimeMillis - s} millis")
    //    s = System.currentTimeMillis()
    //    val lineSelectors = lineIndices.map(SepSelector.fromIndices).filterNot(_.isEmpty)
    //    val lineSelector = lineSelectors.reduce(_ merge _)
      ...
      if (hexes.isEmpty) Nil else
        hexes.flatMap { sh =>
          val maybeStroke = if (curLevel != sh.levelInfo.level) {
            ...
            maybeEndStroke ++ List(s"""<g stroke="rgb(182,255,158)" stroke-width="$stroke">""")
          } else Nil
          val css = sh.corners.toList.map(p =>
            s"${p.x},${transformY(p.y)}"
          )
          maybeStroke ++
            List(s"""<path fill="rgb(255,255,255)" d="M ${css.head} L ${css.tail.mkString(" ")} Z" />""")
        } ++ List("</g>")
    }
