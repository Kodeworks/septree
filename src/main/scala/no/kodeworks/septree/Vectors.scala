...
object Mat33 {
  def skewX(angle: Double): Mat33 = ???

  def apply(a: Double, b: Double, c: Double, d: Double, e: Double, f: Double, g: Double, h: Double, i: Double): Mat33 =
    apply(Vec3(a, b, c), Vec3(d, e, f), Vec3(g, h, i))

  def rotate(angle: Double): Mat33 = ???

  def translate(tx: Double, d: Double): Mat33 = ???

  def scale(sx: Double, d: Double): Mat33 = ???

  val identity = Mat33(Vec3(1d, 0d, 0d), Vec3(0d, 1d, 0d), Vec3(0d, 0d, 1d))
}
